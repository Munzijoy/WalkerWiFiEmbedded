...
ESP8266Interface::ESP8266Interface(PinName tx, PinName rx, uint8_t mode, bool debug)
    :
        _esp(tx, rx, mode, debug)
{
...
int ESP8266Interface::socket_close(void *handle)
{
    ...
    if (!_esp.close(static_cast<esp8266_socket *>(handle)->id)) {
        err = NSAPI_ERROR_DEVICE_ERROR;
    }

    _ids[static_cast<esp8266_socket *>(handle)->id] = false;
    delete static_cast<esp8266_socket *>(handle);
    return err;
}
...
int ESP8266Interface::socket_bind(void *handle, const SocketAddress &address)
{
    ...
    if (!_esp._bServerStarted) {
        if (!_esp.SetupServer(address.get_port())) {
            return NSAPI_ERROR_DEVICE_ERROR;
        }
    }

    static_cast<esp8266_socket *>(handle)->addr = address;
    static_cast<esp8266_socket *>(handle)->proto = NSAPI_TCP;
...
{
    if (!(struct esp8266_socket *) handle) {
        return NSAPI_ERROR_NO_SOCKET;
    } else {
        return (backlog == 1) ? NSAPI_ERROR_OK : NSAPI_ERROR_PARAMETER; // only one connection possible right now
    }
}
...
{
    nsapi_error retVal;
    _esp.SetAPConnectionTimeout(ESP8266_CONNECT_TIMEOUT / 1000);
    ip4_addr resultIp;
    retVal = _esp.WaitForClients(resultIp) ? NSAPI_ERROR_OK : NSAPI_ERROR_WOULD_BLOCK;
    _connectedIp = resultIp;

    esp8266_socket* newSocket = new esp8266_socket;
    SocketAddress address(ip4addr_ntoa(&resultIp));
    newSocket->addr = address;
    newSocket->connected = true;
    newSocket->id = 0; //???
    newSocket->proto = NSAPI_TCP;
    *socket = newSocket;
...
int ESP8266Interface::socket_send(void *handle, const void *data, unsigned size)
{
    ...
    if (!_esp.send(static_cast<esp8266_socket *>(handle)->id, data, size)) {
        return NSAPI_ERROR_DEVICE_ERROR;
    }
...
int ESP8266Interface::socket_recv(void *handle, void *data, unsigned size)
{
    ...
    int32_t recv = _esp.recv(static_cast<esp8266_socket *>(handle)->id, data, size);
    if (recv < 0) {
